...
class MenuScene: SKScene {
    ...
    func setupUI() {
        ...
        charactersButton.position = CGPoint(x:size.width/2, y:size.width/2 + 50)
        ...
        soundButton.position = CGPoint(x: size.width / 2, y: size.height / 2 - 100)
    ...
    func createButton(size: CGSize, text: String) -> SKSpriteNode {
        ...
        label.text = text
        label.fontSize = size.height / 2.5
